		...
		<div
			...
			<div class="cpt-field-inner">
				...
				printf(
					...
					$field_type::render( $input_name, $input_id, $field_config ), //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	...
	private function get_fields_section( $config = array(), $get_value_callback = null ) {
		...
		<div class="cpt-fields-section" data-id="<?php echo esc_html( $fields_group_id ); ?>">
			...
			foreach ( $fields as $field ) {
				...
				echo $this->get_field_template( $field ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
			?>
		</div>
		<?php
				...
				add_meta_box(
					...
					function ( $post ) use ( $field_group ) {
						...
						echo $this->get_fields_section(
							$field_group,
							function ( $key ) use ( $post ) { //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
								return ! empty( $post->ID ) ? get_post_meta( $post->ID, $key, true ) : null; //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
							}
						);
					},
					$post_type,
		...
		foreach ( $add_actions as $action ) {
			add_action(
				...
				function ( $term ) use ( $field_group ) {
					...
					echo $this->get_fields_section(
						$field_group,
						function ( $key ) use ( $term ) { //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
							return ! empty( $term->term_id ) ? get_term_meta( $term->term_id, $key, true ) : null; //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						}
					);
				}
			);
			...
			function () use ( $options_page, $field_group ) {
				...
				add_settings_section(
					...
					function () use ( $options_page, $field_group ) {
						...
						echo $this->get_fields_section(
							$field_group,
							function ( $key ) { //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		...
		foreach ( $add_actions as $action ) {
			add_action(
				...
				function ( $user ) use ( $field_group ) {
					...
					echo $this->get_fields_section(
						$field_group,
						function ( $key ) use ( $user ) { //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
							return ! empty( $user->ID ) ? get_user_meta( $user->ID, $key, true ) : null; //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						}
					);
				}
			);
			...
			function ( $post_type ) use ( $field_group ) {
				...
				add_meta_box(
					...
					function ( $comment ) use ( $field_group ) {
						...
						echo $this->get_fields_section(
							$field_group,
							function ( $key ) use ( $comment ) { //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
								return ! empty( $comment->comment_ID ) ? get_comment_meta( $comment->comment_ID, $key, true ) : null; //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
							}
						);
					},
					'comment',
	...
	public function init_menu_item_fields( $field_group = array() ) {
		...
		add_action(
			...
			function ( $item_id ) use ( $field_group ) {
				...
				echo $this->get_fields_section(
					$field_group,
					function ( $key ) use ( $item_id ) { //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						$key = str_replace( '-' . $item_id, '', $key ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
